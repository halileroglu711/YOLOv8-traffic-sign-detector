...
- The model was trained for 10 epochs and at the end of each epoch, it was tested with validation dataset. The image below illustrates the change in the model's training and validation losses in time.
...
- *mAP50(B)* graph is the most important measurement since it evaluates the model's performance according to a strict rule that a prediction's IoU (intersection/union) value must be higher than 0.50. Also `best.pt` is selected as the weight that has the highest *mAP50* value. *mAP50-95(B)* graph evaluates the model's performance under a stricter rule that it computes the average of the model performance according to *mAP* scores 50 to 95.

# 📥Installation
- In order to install repository and libraries to use on your local computer, follow the steps below: <br>
